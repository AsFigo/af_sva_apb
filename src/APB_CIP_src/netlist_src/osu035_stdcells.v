...
`else
module udp_dff (out, in, clk, clr, set, NOTIFIER);
   output out;
   input  in, clk, clr, set, NOTIFIER;
   reg    out;

endmodule : udp_dff 
module udp_tlat (out, in, enable, clr, set, NOTIFIER);
...
module udp_mux2 (out, in0, in1, sel);
   output out;
   input  in0, in1, sel;
endmodule : udp_mux2 
...
reg NOTIFIER ;
   ...
   not (I0_CLOCK, CLK);
   udp_dff u_vlt_dff (DS0000, D, I0_CLOCK, 1'B0, 1'B0, NOTIFIER);
   not (P0002, DS0000);
   buf (Q, DS0000);
...
reg NOTIFIER ;
   ...
   udp_dff u_vlt_dff (DS0000, D, CLK, 1'B0, 1'B0, NOTIFIER);
   not (P0002, DS0000);
   buf (Q, DS0000);
...
reg NOTIFIER ;
   ...
   not (I0_CLEAR, R);
   not (I0_SET, S);
   udp_dff u_vlt_dff (P0003, D_, CLK, I0_SET, I0_CLEAR, NOTIFIER);
   not (D_, D);
   not (P0002, P0003);
   buf (Q, P0002);
   and (\D&S , D, S);
...
reg NOTIFIER ;
   ...
   udp_tlat u_vlt_tlat (DS0000, D, CLK, 1'B0, 1'B0, NOTIFIER);
   not (P0000, DS0000);
   buf (Q, DS0000);
...
output Y ;
   ...
   udp_mux2 u_vlt_mux2 (I0_out, B, A, S);
   not (Y, I0_out);
